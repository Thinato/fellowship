#!/bin/sh
# Installs the latest fellowship release from GitHub.
#
# Usage:
...
#   FELLOWSHIP_INSTALL_DIR  install location (default: $HOME/.local/bin)
#   FELLOWSHIP_VERSION      release tag to fetch (default: latest, e.g. v0.1.0)

set -eu

REPO="Thinato/fellowship"
INSTALL_DIR="${FELLOWSHIP_INSTALL_DIR:-$HOME/.local/bin}"
VERSION="${FELLOWSHIP_VERSION:-latest}"
...
main() {
    ...
    tag="$(resolve_version)"
    name="fellowship-${tag}-${target}"
    url="https://github.com/${REPO}/releases/download/${tag}/${name}.tar.gz"

    info "fellowship ${tag} (${target})"
